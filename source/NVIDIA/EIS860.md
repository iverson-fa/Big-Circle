...
index a75234b8b..591842a5b 100644
...
显示驱动文件：`Linux_for_Tegra/source/public/nvidia_kernel_display_driver_source.tbz2`

```shell
# in Linux_for_Tegra/source/public/
$ tar xfp nvidia_kernel_display_driver_source.tbz2
$ cd NVIDIA-kernel-module-source-TempVersion
# build
$ make modules -j12 SYSSRC=/home/dafa/jetson_flash/r35.4/BSP/Linux_for_Tegra/source/public/kernel/kernel-5.10 \
SYSOUT=/home/dafa/jetson_flash/r35.4/BSP/kernel_out \
CC=/opt/l4t-gcc/bin/aarch64-buildroot-linux-gnu-gcc \
LD=/opt/l4t-gcc/bin/aarch64-buildroot-linux-gnu-ld \
AR=/opt/l4t-gcc/bin/aarch64-buildroot-linux-gnu-ar \
CXX=/opt/l4t-gcc/bin/aarch64-buildroot-linux-gnu-g++ \
OBJCOPY=/opt/l4t-gcc/bin/aarch64-buildroot-linux-gnu-objcopy \
TARGET_ARCH=aarch64 ARCH=arm64 NV_VERBOSE=1
```

最后不要忘记`make ARCH=arm64 O=$TEGRA_KERNEL_OUT modules_install INSTALL_MOD_PATH=$WS/Linux_for_Tegra/rootfs/`。

### 8.2 rootfs

```shell
# /boot/extlinux/extlinux.conf APPEND所在行末尾追加
video=efifb:off console=tty0
```
...
## JP6.x
echo 'host' > /sys/devices/platform/bus@0/3520000.padctl/usb2-0/usb_role/usb2-0-role-switch/role
```
...
或在自启脚本中添加：`echo -1 > /sys/module/usbcore/parameters/autosuspend`
...
```shell
# In /lib/modules/5.10.120-tegra
rm build
ln -s /usr/src/linux-headers-5.10.120-tegra-ubuntu20.04_aarch64/kernel-5.10/ build
# JP 6.2
rm /lib/modules/5.15.148-tegra/build
ln -s /usr/src/linux-headers-5.15.148-tegra-ubuntu22.04_aarch64/3rdparty/canonical/linux-jammy/kernel-source/ /lib/modules/5.15.148-tegra/build
```
...
EIS860_VERSION="3.1"
# /etc/lsb-release 修改
DISTRIB_DESCRIPTION="EIS860 V3.1"
```
...
2.11版本修复了 `org.freedesktop.systemd1` 激活失败的系统日志报错，需要将显示器管理器由gdm3改为lightdm:

```bash
dpkg-reconfigure lightdm
```

2.11版本后，IECP默认不开启，需要在BSP中删除自启动服务文件：

```shell
rm /etc/systemd/system/multi-user.target.wants/inspurnode.service
rm /etc/systemd/system/multi-user.target.wants/ecma.service
rm /etc/systemd/system/multi-user.target.wants/ecmavnc.service
```

3.0版本后，上述服务文件默认不生成，不用额外删除。
...
5.参考[官方文档](https://docs.nvidia.com/jetson/archives/r35.4.1/DeveloperGuide/text/SD/Bootloader/UEFI.html?highlight=uefi#customizing-the-default-boot-order-in-l4tconfiguration-dtbo-in-the-bsp-directory)修改系统引导项顺序：emmc,pxev4,usb,nvme,sd,ufs,sata,httpv4,scsi,pxev6,httpv6,cdrom,boot.img
...
```shell
# 安装module
install -p -m 644 qmi_wwan_f.ko /lib/modules/5.10.120-tegra/kernel/drivers/net/wireless/
install -p -m 644 qmi_wwan_q.ko /lib/modules/5.10.120-tegra/kernel/drivers/net/wireless/
```
...
Bus 001 Device 004: ID 10c4:ea71 Silicon Labs CP2108 Quad UART Bridge
...
新的PCN更新了emmc等部件的供应商，需要修改一个文件：

- `Linux_for_Tegra/bootloader/t186ref/cfg/flash_t234_qspi_sdmmc.xml`

将`num_sectors`的值由 124321792 改为 124190720，这个值由基于BSP烧录完进OS执行以下命令得到

```shell
root@eis860:/home/orin# fdisk -l
Disk /dev/mmcblk0: 59.22 GiB, 63585648640 bytes, 124190720 sectors
Units: sectors of 1 * 512 = 512 bytes
Sector size (logical/physical): 512 bytes / 512 bytes
I/O size (minimum/optimal): 512 bytes / 512 bytes
Disklabel type: gpt
Disk identifier: 52CF76E1-F2CE-4E0A-A194-7E5EF9CCA55C
```
