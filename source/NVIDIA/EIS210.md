...
在BSP目录`Linux_for_Tegra/source/public/kernel/kernel-5.10/tools/spi `下，执行`make ARCH=arm64` 生成`spidev_test`，替换音频目录。
...
[升级sudo组件](https://big-circle.readthedocs.io/en/latest/NVIDIA/EIS860.html#id9)。
...
```shell
install -p -m 644 8852be.ko /lib/modules/5.10.104-tegra/kernel/drivers/net/wireless/
